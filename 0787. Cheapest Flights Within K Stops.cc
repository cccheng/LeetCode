...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int K) {
        ...
        vector<vector<int>> price(K+1, vector<int>(n+1, numeric_limits<int>::max())); // stop, node
        ...
        while (!pq.empty()) {
            int stop, node, cur_price;
            tie(stop, node, cur_price) = pq.top();
            pq.pop();
            
            if (stop > K)
                continue;
            if (price[stop][node] < cur_price)
                continue;
            price[stop][node] = cur_price;
